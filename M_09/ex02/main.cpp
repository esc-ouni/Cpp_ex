...
int main(int argc, char *argv[]){
    clock_t n, n1, n2, n3, _n, _n1, _n2, _n3,  nt, nt2;
    std::deque<int>  Input;
    std::deque<int>  deque;
    ...
    try{
        ...
        _n =  run_using_deque(Input, deque);
        deque.clear();
        _n1 = run_using_deque(Input, deque);
        deque.clear();
        _n2 = run_using_deque(Input, deque);
        deque.clear();
        _n3 = run_using_deque(Input, deque);
        
        nt2 = ((double)(_n1 + _n2 + _n3) / 3);
    }
    ...
    // std::cout << "estimatation   ~: " << spent_time(nt) << " µs" << std::endl << std::endl;

    // std::cout << "=> Ford-Johnson (deque): size : " << deque.size() << std::endl;
    // std::cout << "Is sorted : " << std::boolalpha << std::is_sorted(deque.begin(), deque.end()) << std::endl;
    // std::cout << "Time to process : " << spent_time(_n)<< " µs" << std::endl;
    // std::cout << "Time to process : " << spent_time(_n1) << " µs" << std::endl;
    // std::cout << "Time to process : " << spent_time(_n2) << " µs" << std::endl;
    // std::cout << "Time to process : " << spent_time(_n3) << " µs" << std::endl << std::endl;
